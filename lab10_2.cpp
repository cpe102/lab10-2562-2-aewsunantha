...
int calScore(int x,int y,int z){
    ...
    int sum=0;
		...
		sum=(x+y+z)%10;
	/*	 if(x<10) sum=sum+x;
		else if(x<14) sum=sum+10;
		
	
		 if(y<10) sum=sum+y;
		else if(y<14 ) sum=sum+10;
		
		
		 if(z<10) sum=sum+z;
		else if(z<14 ) sum=sum+10;
...
int findYugiAction(int s){	
	if(s == 9) return 2; 
	// Yugi will definitely stay (2) when current score (s) is equal to 9
...
int main(){	
	srand(time(0));
	int playerScore, yugiScore, playerAction, yugiAction,newscore,yourCard;
